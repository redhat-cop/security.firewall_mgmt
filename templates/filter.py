...
def filter_on_source_ip(gathered_result, source_ip, match_file, hit_cnt_file):
    ...
    for each in gathered_result['acls']:
        for every in each['aces']:
            if every.get('source') and every['source'].get('address') and every['source']['address'] == source_ip:
                if each['name'] not in temp_match_name:
                    match.append({'name': each['name'], 'acl_type': each['acl_type'], 'aces': [every]})
    ...
    for each in match:
        for line in each['aces']:
            hit_line = line['line']
            cmd_hitcount = 'show access-list | inc {0} line {1}'.format(each['name'], hit_line)
...
def main():
    ...
    filter_cmd = filter_on_source_ip(test_gather['gathered'], filter_ip, match_file, hit_cnt_file)
